...
**Нормализованная база данных университета (3NF, 8 таблиц). Включает веб-интерфейс (Streamlit), REST API (FastAPI) и контейнеризацию (Docker).**
...
Автоматически строится для **любой** PostgreSQL БД через `information_schema` + `WITH RECURSIVE`:

### Результат для этой БД:
![Граф зависимостей](screenshots/graph.png)

### Особенности:
- **Работает для любой БД** — не нужно знать структуру заранее
- **Определяет глубину** — минимальное расстояние от корневых таблиц
- **Находит всех родителей** — даже если таблица имеет два FK (как `grades`)
- **Защита от циклов** — не уходит в бесконечную рекурсию

Запустить:
```bash
psql -U postgres -d mf_mgtu -f sql/graph.sql
```
##
---
